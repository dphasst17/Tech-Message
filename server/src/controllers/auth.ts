...
export default class Auth {
  ...
  private createToken = (idUser: string) => {
    const token = jwt.sign({ id: idUser }, process.env.SECRET_KEY as string, {
      expiresIn: "5d",
    });
  ...
  public login = (req: Request, res: Response) => {
    ...
    collectionAuth
      .find({ username: resultData.username })
      .toArray()
      .then((results) => {
        if (results.length === 0) {
  ...
  public register = async (req: Request, res: Response) => {
    ...
    try {
      ...
      await collectionAuth.insertOne({
      ...
      collectionInfo
        ...
        .then((infoRes) =>
          NewResponse.responseMessage(res, 201, "Register is success")
        );
    } catch (error) {
      console.log(error);
      NewResponse.responseMessage(
